...
import { evaluateBehavioralResponse, generateFollowUp } from '../utils/openai';
...
export default function BehavioralInterview() {
 ...
 useEffect(() => {
  if (questions.length > 0) return; // Already loaded, don't reload
  
  // Select random questions ONCE
  ...
  const handleResponse = async (response) => {
    try {
      ...
      // Generate follow-up or move to next
      const followUp = await generateFollowUp(questions[currentQuestionIndex], response);
      
      // Check if should continue or move on
      if (response.length > 100 || newHistory.filter(h => h.question === questions[currentQuestionIndex]).length >= 2) {
        // Move to next question or finish
        if (currentQuestionIndex < questions.length - 1) {
          await speakText(`${followUp} Let's move to the next question.`);
